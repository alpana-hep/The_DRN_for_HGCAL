...
#                ys+=[(transforms[mod.target](mod.y_pred, mod.data['rawE'], mod.data['trueE'])[mod.valid_idx])]
    ...
    def loadSigma(self):
        ...
        '''
        prediction is factor+-sigma
        Epred = Eraw * (factor+-sigma)
        sigmapred = sigma*Eraw
        sigma/Epred = sigma*Eraw/Epred 
                    = sigma*Eraw/(Eraw * factor)
                    = sigma/factor
        '''
    ...
    def plotError(self):
        ...
        x = (target - self.y_pred)/self.sigma
        ...
        props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
        textstr = 'Filtered to $\pm 3 \sigma$\n$\mu = %.3f$\n$\sigma = %.3f$'%(mean, var)
        plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=14, verticalalignment='top', bbox=props)

        plt.xlabel(r"$(y_{True} - y_{Pred})/\sigma_{Pred}$")
        plt.ylabel("Count")

        plt.savefig("%s/error.png"%self.folder)
        plt.clf()

    @staticmethod 
    def compareSigmaDist(folders, labels, idxs, data_folders, outdir, fname):
        pass
